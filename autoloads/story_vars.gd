...
## Remet ce bac à zéro (voir save_manager.gd::delete_save) — un autoload
...
## directement sur le bureau "post-rapport" de la mission 1). Simple alias de
## load_all({}) plutôt qu'une seconde implémentation du même vidage.
func reset() -> void:
	load_all({})
